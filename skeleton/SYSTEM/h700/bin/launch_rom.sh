...
if [ "$THISRUN" = "minarch" ]; then
    ...
    HOME="$USERDATA_PATH"
    cd "$HOME"
    LD_PRELOAD=${SDCARD_PATH}/.system/h700/lib/libredirect_dsp.so SDL_AUDIODRIVER=dsp SDL_PATH_DSP=/tmp/dsp minarch.elf "$CORES_PATH/${EMU_EXE}_libretro.so" "$ROM" $2 > "$LOGS_PATH/$EMU_TAG.txt"
fi
