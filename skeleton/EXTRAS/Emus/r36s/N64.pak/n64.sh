...
if [ $yres -gt $xres ]; then
  #swap them if the display is in portrait mode
  temp=$xres
  xres=$yres
  yres=$temp
fi
...
if [[ $1 == "standalone-Rice" ]]; then
  if [[ $2 == "Widescreen_Aspect" ]]; then
    sudo -u ark /opt/mupen64plus/mupen64plus --resolution "${xres}x${yres}" --plugindir /opt/mupen64plus --gfx mupen64plus-video-rice.so --corelib /opt/mupen64plus/libmupen64plus.so.2 --datadir /opt/mupen64plus "$game"
  else
    ricewidthhack=$(((yres * 4) / 3))
    sudo -u ark /opt/mupen64plus/mupen64plus --resolution "${ricewidthhack}x${yres}" --plugindir /opt/mupen64plus --gfx mupen64plus-video-rice.so --corelib /opt/mupen64plus/libmupen64plus.so.2 --datadir /opt/mupen64plus "$game"
  fi
elif [[ $1 == "standalone-Glide64mk2" ]]; then
  if [[ $2 == "Widescreen_Aspect" ]]; then
    sudo -u ark /opt/mupen64plus/mupen64plus --set Video-Glide64mk2[aspect]=1 --resolution "${xres}x${yres}" --plugindir /opt/mupen64plus --gfx mupen64plus-video-glide64mk2.so --corelib /opt/mupen64plus/libmupen64plus.so.2 --datadir /opt/mupen64plus "$game"
  else
    sudo -u ark /opt/mupen64plus/mupen64plus --set Video-Glide64mk2[aspect]=-1 --resolution "${xres}x${yres}" --plugindir /opt/mupen64plus --gfx mupen64plus-video-glide64mk2.so --corelib /opt/mupen64plus/libmupen64plus.so.2 --datadir /opt/mupen64plus "$game"
  fi
elif [[ $1 == "standalone-GlideN64" ]]; then
  if [[ $2 == "Widescreen_Aspect" ]]; then
    sudo -u ark /opt/mupen64plus/mupen64plus --set Video-GLideN64[ThreadedVideo]=True --set Video-GLideN64[UseNativeResolutionFactor]=1 --set Video-GLideN64[AspectRatio]=3 --resolution "${xres}x${yres}" --plugindir /opt/mupen64plus --gfx mupen64plus-video-GLideN64.so --corelib /opt/mupen64plus/libmupen64plus.so.2 --datadir /opt/mupen64plus "$game"
  else
  ...
  sudo -u ark /usr/local/bin/"$1" -L /home/ark/.config/"$1"/cores/"$2"_libretro.so "$3"
fi
if [ -f "${KILLSTANDALONE_PATH}" ]; then
  sudo rm "${KILLSTANDALONE_PATH}"
fi
