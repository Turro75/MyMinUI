...
### All:
- Added the feature Play as Player X, once in game enter the menu->frontend then select the Play as Player 1-2-3-4 to move the controller to one of the 4 players.
  This allow to impersonate a different player in some arcade games (simpson, nbajam,etc..) You can change it at any time during the game.
- refactoring of the rotate frame algo used to transform/move the frame data to another internal buffer before rendering to the screen, this new algo now rotates small tiles then recombine them allowing even soc with poor cache performances (i.e. allwinner H133 and RK3326) allowing them to easily handle frames which are generated by the core already big (i.e. GranTurismo2 garage menu used to stutter while the race was perfect due to that). Anyway this turned out as a great improvements for all systems.   
- I locked the git version of all cores to ensure better stability during compilation process and core compatibility 

### m21/m22pro:
- enabled and refined the rendering engine that now uses the layers (for real this time) allowing double buffering to reduce the screen tearing effect.
- added a screen colorbar appearing during program start and during the transition from one program to another
- added a way to edit the system button mapping, just edit the file .system/m21/custombuttonmapping.env where You can find the physical button code and You can swap the meaning of every button by enabling some system shell variables. This will affect also the in game mapping as it changes the reference system buttons. in theory You can assign the power button function to volume- if You like.
- I discovered it allows the use of the hardware vsync while in hdmi, I enabled it so it now can run at 60fps most of the supported game avoiding screen tearing when connected to an external monitor. Unfortunately due to the low performances of the onboard screen used apparently there is no way to get vsync working without affecting fps (38fps on m22pro, 51fps on m21). So vsync settings is read as off in m21/m22pro regardless the user setting when on local screen while it follow the user setting once in hdmi output mode. The solution would be patching the dtb files (without kernel sources) to edit the screen freq and/or the screen resolution (1920x1080 on m22pro and 1280x720 on m21). If someone know how to do that we can try to fix it.

### Miyoomini:
- the menu is now downscaled to 640x480 even in v4 which is 752x560, the game stay 752x560.
- reduced the max cpu freq down to 1.6GHz as 1.7GHz sometimes turned into system freezes.
