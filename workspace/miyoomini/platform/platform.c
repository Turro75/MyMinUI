...
int MM_SDLFB_FlipRotate180(SDL_Surface *buffer, void * fbmmap, int linewidth) {
...
int MM_SDLFB_Flip(SDL_Surface *buffer, void * fbmmap, int linewidth) {
	//copy a surface to the screen and flip it
	//it must be the same resolution, the bpp16 is then converted to 32bpp
	//fprintf(stdout,"Buffer has %d bpp\n", buffer->format->BitsPerPixel);fflush(stdout);
...
int MM_SDLFB_FlipRotate90(SDL_Surface *buffer, void * fbmmap, int linewidth) {
	//copy a surface to the screen and flip it
	//it must be the same resolution, the bpp16 is then converted to 32bpp
	//fprintf(stdout,"Buffer has %d bpp\n", buffer->format->BitsPerPixel);fflush(stdout);
...
int MM_SDLFB_FlipRotate270(SDL_Surface *buffer, void * fbmmap, int linewidth) {
	//copy a surface to the screen and flip it
	//it must be the same resolution, the bpp16 is then converted to 32bpp
	//fprintf(stdout,"Buffer has %d bpp\n", buffer->format->BitsPerPixel);fflush(stdout);
...
//	if (sync) PLAT_vsync(FRAME_BUDGET - (SDL_GetTicks()-frame_start));
	...
	{
		// No Rotation
		MM_SDLFB_Flip(vid.screen, vid.fbmmap,vid.linewidth);
	}
	if (vid.rotate == 1)
	{
		// 90 Rotation
		MM_SDLFB_FlipRotate90(vid.screen, vid.fbmmap,vid.linewidth);
	}
	if (vid.rotate == 2)
	{
		// 180 Rotation
		MM_SDLFB_FlipRotate180(vid.screen, vid.fbmmap,vid.linewidth);
	}
	if (vid.rotate == 3)
	{
		// 270 Rotation
		MM_SDLFB_FlipRotate270(vid.screen, vid.fbmmap,vid.linewidth);
	}
