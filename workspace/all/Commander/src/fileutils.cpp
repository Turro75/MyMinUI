...
void File_utils::diskInfo(void)
{
    ...
    std::string mydisk2 = std::string("df -h ");
    std::string mydisk = getenv("MYDISK");
    if (mydisk.empty())
    {
        mydisk = std::string(FILE_SYSTEM);
    }
    mydisk2 += mydisk;
    // Execute command df -h
    {
        
        //mydisk2 += std::string(getenv("MYDISK"));        
        //std::string mydisk2 = std::string( "df -h") + std::string(getenv("MYDISK"));
        char l_buffer[256];
        FILE *l_pipe = popen(mydisk2.c_str() , "r");
        if (l_pipe == NULL)
        {
        ...
        while (
            ...
            if (strstr(l_buffer, mydisk.c_str() ) != NULL) l_line = l_buffer;
        pclose(l_pipe);
    }
    if (!l_line.empty())
    {
        ...
        ErrorDialog(
            "Error getting disk info ",  mydisk.c_str(), " not found");
}
